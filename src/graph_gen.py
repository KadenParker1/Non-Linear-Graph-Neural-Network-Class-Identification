...
# def generate_graph2(f1,f2, num_samples, num_classes,noise,lambdav,degree,separation):
#     """
#     Generates adjacency matrix for the graph by placing edges between nodes
...
# def generate_coords2(f1=t,f2=t+np.pi, num_samples, num_classes, noise,separation, mode="polar"):
#     """
#     Generates the positions and classes of the nodes in the graph concentrically
...
#             # Using a polar function
#             if mode == "polar":
#                 radius1=np.random.normal(loc=separation*radii[class_index],scale=noise)
#                 angle1=np.random.uniform(low=0,high=2*np.pi)
#                 pos[i, 0] = radius1 * f1(angle1) * np.cos(angle1)
#                 pos[i, 1] = radius1 * f1(angle1) * np.sin(angle1)
#                 classes[i] = class_index

#                 radius2=np.random.normal(loc=separation*radii[class_index],scale=noise)
#                 angle2=np.random.uniform(low=0,high=2*np.pi)
#                 pos[i, 0] = radius1 * f2(angle2) * np.cos(angle2)
#                 pos[i, 1] = radius1 * f2(angle2) * np.sin(angle2)
#                 classes[i] = class_index
